...
@test "roxctl-development netpol connectivity diff generates conns diff report between resources from directories with admin network policies md output" {
    dir1="${test_data}/np-guard/anp_banp_demo"
    dir2="${test_data}/np-guard/anp_banp_diff_demo"
    # assert files exist in dir1
    assert_file_exist "${dir1}/ns.yaml"
    ...
    assert_output --partial '| diff-type | source | destination | dir1 | dir2 | workloads-diff-info |
|-----------|--------|-------------|------|------|---------------------|
| added | baz/mybaz[Pod] | bar/mybar[Pod] | No Connections | All Connections |  |
| added | foo/myfoo[Pod] | bar/mybar[Pod] | No Connections | All Connections |  |
| removed | monitoring/mymonitoring[Pod] | foo/myfoo[Pod] | All Connections | No Connections |  |'
}

@test "roxctl-development netpol connectivity diff generates conns diff report between resources from directories with admin network policies csv output" {
    dir1="${test_data}/np-guard/anp_banp_demo"
    dir2="${test_data}/np-guard/anp_banp_diff_demo"
    # assert files exist in dir1
    assert_file_exist "${dir1}/ns.yaml"
